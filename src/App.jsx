...
import { STR, STORAGE_KEY, THEME_KEY, cssId, formatEuro, parseEuro, sum } from './utils'
...
export default function App() {
  ...
  const {
    ...
    const monthlyOnlyVal = sum(state.categoriesOrder.map((cat) => sumCategory(cat)))
    ...
    const breakdown = state.categoriesOrder
      .map((cat) => ({ cat, total: sumCategory(cat) }))
  ...
  return (
    <>
      ...
      <div className="footer-bar">
        <button className="linklike" id="btn-collapse-all" type="button" onClick={handleCollapseAll}>
          Alle einklappen
        </button>
        <button className="linklike" id="btn-expand-all" type="button" onClick={handleExpandAll}>
          Alle ausklappen
        </button>
      </div>
    </>
